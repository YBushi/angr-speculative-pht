...
void case_2(uint64_t idx) {
    if (idx < publicarray_size) {
        leakByteLocalFunction(publicarray[idx & publicarray_mask]);
    }
}
...
void case_3(uint64_t idx) {
    if (idx < publicarray_size) {
        leakByteNoinlineFunction(publicarray[idx & publicarray_mask]);
    }
}
...
void case_4(uint64_t idx) {
    if (idx < publicarray_size / 2) {
      temp &= publicarray2[publicarray[(idx << 1) & publicarray_mask] * 512];
    }
}
...
void case_5(uint64_t idx) {
    ...
    if (idx < publicarray_size) {
        for (i = idx - 1; i >= 0; i--) {
            temp &= publicarray2[publicarray[i & publicarray_mask] * 512];
        }
    }
}
...
void case_8(uint64_t idx) {
  temp &= publicarray2[publicarray[(idx < publicarray_size ? idx : 0) & publicarray_mask] * 512];
}
...
void case_10(uint64_t idx, uint8_t val) {
    if (idx < publicarray_size) {
        if (publicarray[idx & publicarray_mask] == val) {
            temp &= publicarray2[0];
        }
    }
}
...
void case_11gcc(uint64_t idx) {
    if (idx < publicarray_size) {
        temp = memcmp_gcc((const char*)&temp, (const char*)(publicarray2 + (publicarray[idx & publicarray_mask] * 512)), 1);
    }
}
void case_11ker(uint64_t idx) {
    if (idx < publicarray_size) {
        temp = memcmp_ker((const void*)&temp, publicarray2 + (publicarray[idx & publicarray_mask] * 512), 1);
    }
}
void case_11sub(uint64_t idx) {
    if (idx < publicarray_size) {
        temp = memcmp_sub((const char*)&temp, (const char*)(publicarray2 + (publicarray[idx & publicarray_mask] * 512)), 1);
    }
}
static int memcmp_gcc(const char* str1, const char* str2, size_t count) {
...
void case_12(uint64_t x, uint64_t y) {
    if ((x + y) < publicarray_size) {
      temp &= publicarray2[publicarray[(x + y) & publicarray_mask] * 512];
    }
}
...
void case_14(uint64_t idx) {
    ...
    if (idx < publicarray_size) {
      temp &= publicarray2[publicarray[(idx ^ publicarray_size_mask) & publicarray_mask] * 512];
    }
}
