...
class MemoryItem(object):
    ...
    def _compare_obj(self, other):
        ...
        if isinstance(self._obj, list) and isinstance(other._obj, list) \
                and id(self._obj[0]) == id(other._obj[0]) \
                and self._obj[1] == self._obj[1]:
            return True

        if isinstance(self._obj, list):
            if not isinstance(self._obj[0], claripy.ast.bv.BV):
                return False
        elif not isinstance(self._obj, claripy.ast.bv.BV):
            return False

        if isinstance(other._obj, list):
            if not isinstance(other._obj[0], claripy.ast.bv.BV):
                return False
        elif not isinstance(other._obj, claripy.ast.bv.BV):
            return False
...
class FullySymbolicMemory(SimMemory):
    ...
    def merge(self, others, merge_conditions, common_ancestor=None):
        ...
        if isinstance(common_ancestor, SimStateHistory):
    ...
    def _merge_concrete_memory(self, other, merge_conditions):
        ...
        for page_index in page_indexes:
            ...
            for offset in offsets:
                ...
                if not isinstance(v_self, list) and not isinstance(v_other, list):
                ...
                if v_other is None and v_self is not None and not isinstance(v_self, list) and v_self.t == 0 and v_self.guard is None:
                    same_value = True

                # Symmetric case. We need to insert in self.
                if v_self is None and v_other is not None and not isinstance(v_other, list) and v_other.t == 0 and v_other.guard is None:
